...
![version](https://img.shields.io/badge/version-1.6.0-C0392B) · 현재 버전 **v1.6.0** · [변경 이력](CHANGELOG.md)

슬라이드/페이지 단위로 구성된 **HTML 보고서를 페이지별로 편집**하는 Windows 데스크톱 에디터입니다.
슬라이드 구조가 아닌 **일반 HTML 문서**도 **문서 모드**로 열어 전체를 편집할 수 있습니다.
...
- **문서 모드** — 슬라이드 구조가 없는 일반 HTML 보고서는 문서 개요(제목 `h1`·`h2`·`h3`)로 탐색하며 전체 문서를 한 화면에서 편집 (파일을 열면 형식을 자동 감지)
...
- **저장 위치 설정**(⚙ 설정) — "지정한 폴더에 자동 저장" 또는 "저장할 때 위치 선택"(설치형 앱)
- **자동 업데이트**(⚙ 설정) — GitHub Releases에서 새 버전 확인 후 자동 다운로드·설치(설치형 앱)
- **전체 미리보기** — 편집한 전체 보고서를 새 창에서 확인

## 지원하는 보고서 형식

**슬라이드형 HTML** — 각 페이지가 다음 중 하나로 구성되면 페이지 단위로 편집합니다.
...
**일반 HTML 문서(문서 모드)** — 위 슬라이드 구조가 없는 HTML은 문서 모드로 열립니다.

- 사이드바가 문서 개요(TOC)로 바뀌어 제목(`h1`·`h2`·`h3`)으로 문서를 탐색
- 전체 문서를 하나의 편집 영역에서 WYSIWYG·소스로 편집(서식·표·이미지 도구 공통)
- 제목이 없으면 "(문서 전체)" 한 항목으로 전체를 편집
